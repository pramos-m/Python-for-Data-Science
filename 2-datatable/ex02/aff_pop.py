...
def ft_plot_countries(df):
    df['Spain'] = df['Spain'].apply(convert_population)
    df['Afghanistan'] = df['Afghanistan'].apply(convert_population)
    ...
    plt.plot(df['index'].values, df['Spain'].values, label='Spain')

    plt.plot(df['index'].values, df['Afghanistan'].values, label='Afghanistan')
